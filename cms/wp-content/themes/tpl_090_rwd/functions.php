...
/* Default Post label
/*---------------------------------------------------------*/
/* unction change_post_menu_label() {
     global $menu, $submenu;
     $menu[5][0] = '応募者'; 
     $submenu['edit.php'][5][0] = '応募者一覧'; 
     $submenu['edit.php'][10][0] = '新規応募者'; 
 }
...
/* 管理画面のメニューから「投稿」を非表示にする
/*---------------------------------------------------------*/
function remove_default_post_menu() {
    remove_menu_page('edit.php'); // 投稿一覧
}
...
function register_scholar_eval_csv_export() {
  add_submenu_page(
      'edit.php?post_type=scholar_eval',     
      'CSVエクスポート',                     
      'CSVエクスポート',                     
      'manage_options',                      
      'export-scholar-eval-csv',             
      'export_scholar_eval_csv_page'         
  );
}
...
function reorganize_admin_menu_with_shortcuts() {
    ...
    if (isset($submenu[$parent_comp])) {
        // 標準の「年度」メニューを消す
        foreach ($submenu[$parent_comp] as $key => $sub_item) {
            if (strpos($sub_item[2], 'taxonomy=evaluation_year') !== false) {
                unset($submenu[$parent_comp][$key]);
            }
        }
        $submenu[$parent_comp][5][0]  = '1_応募者一覧';
        $submenu[$parent_comp][10][0] = '2_応募者新規登録';
    }
    ...
    if (isset($submenu[$parent_audi])) {
        foreach ($submenu[$parent_audi] as $key => $sub_item) {
            if (strpos($sub_item[2], 'taxonomy=evaluation_year') !== false) {
                unset($submenu[$parent_audi][$key]);
            }
        }
        $submenu[$parent_audi][5][0]  = '1_応募者一覧';
        $submenu[$parent_audi][10][0] = '2_応募者新規登録';
    }
    ...
    if (isset($submenu[$parent_scholar])) {
        foreach ($submenu[$parent_scholar] as $key => $sub_item) {
            if (strpos($sub_item[2], 'taxonomy=evaluation_year') !== false) {
                unset($submenu[$parent_scholar][$key]);
            }
        }
        $submenu[$parent_scholar][5][0]  = '1_申請者一覧';
        $submenu[$parent_scholar][10][0] = '2_申請者新規登録';
    }
    ...
    add_submenu_page('common_settings', 'インポート', '1_インポート', 'manage_options', 'admin.php?import=csv');
    add_submenu_page('common_settings', '年度カテゴリ管理', '2_年度カテゴリ', 'manage_options', 'edit-tags.php?taxonomy=evaluation_year');
    if (isset($submenu['common_settings'])) {
        unset($submenu['common_settings'][0]);
    }

    // 不要な元メニューを非表示にする
    remove_menu_page('edit.php?post_type=evaluations');
    remove_menu_page('edit.php?post_type=evaluations_audition');
    remove_menu_page('edit.php?post_type=scholar_eval');
}
...
/******************************************************
 * 4. 評価データ保存時、親から「年度」をコピーしてセット
 *****************************************************/
add_action('save_post', 'force_sync_evaluation_category', 99, 3);
function force_sync_evaluation_category($post_id, $post, $update) {
    $target_types = array('evaluations', 'evaluations_audition', 'scholar_eval');
    if (!in_array($post->post_type, $target_types)) return;

    if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) return;

    $parent_id = get_post_meta($post_id, 'evaluated_post_id', true);

    if (!empty($parent_id)) {
        // 親（応募者）に付いている「年度（evaluation_year）」を取得
        $terms = get_the_terms($parent_id, 'evaluation_year');
        
        if ($terms && !is_wp_error($terms)) {
            $year_ids = array();
            foreach ($terms as $term) {
                $year_ids[] = (int) $term->term_id;
                
                // 【重要】ここを evaluation_category ではなく evaluation_year に修正
                // インポート時に作成されているはずですが、念のため確認
                if (!term_exists($term->name, 'evaluation_year')) {
                    wp_insert_term($term->name, 'evaluation_year');
                }
            }

            // 評価データ自身にも「年度(evaluation_year)」としてセット
            remove_action('save_post', 'force_sync_evaluation_category', 99);
            wp_set_object_terms($post_id, $year_ids, 'evaluation_year');
            add_action('save_post', 'force_sync_evaluation_category', 99, 3);
        }
    }
}
...
function has_user_already_evaluated_audition($user_id, $post_id) {
    ...
    foreach ($evals as $eval) {
        ...
        $stored_user_id = SCF::get('user_id', $eval->ID);
        $stored_post_id = SCF::get('evaluated_post_id', $eval->ID);

        // 型を揃えて比較
        if ( (string)$stored_user_id === (string)$user_id && (string)$stored_post_id === (string)$post_id ) {
            return true;
        }
    }
    return false;
